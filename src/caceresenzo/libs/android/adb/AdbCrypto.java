...
/**
 * This class encapsulates the ADB cryptography functions and provides an interface for the storage and retrieval of keys.
 */
public class AdbCrypto {
	...
	private static byte[] convertRsaPublicKeyToAdbFormat(RSAPublicKey pubkey) {
		 ...
		 * 
		 * @line typedef struct RSAPublicKey {
		 * @line int len; // Length of n[] in number of uint32_t
		 * @line uint32_t n0inv; // -1 / n[0] mod 2^32
		 * @line uint32_t n[RSANUMWORDS]; // modulus as little endian array
		 * @line uint32_t rr[RSANUMWORDS]; // R^2 as little endian array
		 * @line int exponent; // 3 or 65537
		 * @line } RSAPublicKey;
		 */
		...
		ByteBuffer byteBuffer = ByteBuffer.allocate(524).order(ByteOrder.LITTLE_ENDIAN);
		
		byteBuffer.putInt(KEY_LENGTH_WORDS);
		byteBuffer.putInt(n0inv.negate().intValue());
		for (int i : myN) {
			byteBuffer.putInt(i);
		}
		for (int i : myRr) {
		...
		byteBuffer.putInt(pubkey.getPublicExponent().intValue());
