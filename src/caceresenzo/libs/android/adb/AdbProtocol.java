...
public class AdbProtocol {
	 ...
	 */
	public static boolean validateMessage(AdbMessage message) {
		/* Magic is cmd ^ 0xFFFFFFFF */
		if (message.command != (message.magic ^ 0xFFFFFFFF))
			return false;
		
		if (message.payloadLength != 0) {
			if (getPayloadChecksum(message.payload) != message.checksum)
				return false;
		}
	...
	public static byte[] generateMessage(int cmd, int arg0, int arg1, byte[] payload) {
		/**
		 * @line struct message {
		 * @line unsigned command; // command identifier constant
		 * @line unsigned arg0; // first argument
		 * @line unsigned arg1; // second argument
		 * @line unsigned data_length; // length of payload (0 is allowed)
		 * @line unsigned data_check; // checksum of data payload
		 * @line unsigned magic; // command ^ 0xffffffff
		 * @line };
		 */
	...
	public static byte[] generateOpen(int localId, String destination) throws UnsupportedEncodingException {
		...
		return generateMessage(CMD_OPEN, localId, 0, byteBuffer.array());
	}
	...
	final static class AdbMessage {
		...
		public static AdbMessage parseAdbMessage(InputStream in) throws IOException {
			...
			message.command = packet.getInt();
			message.arg0 = packet.getInt();
			message.arg1 = packet.getInt();
			message.payloadLength = packet.getInt();
			message.checksum = packet.getInt();
			message.magic = packet.getInt();
			...
			if (message.payloadLength != 0) {
				...
				dataRead = 0;
				do {
					int bytesRead = in.read(message.payload, dataRead, message.payloadLength - dataRead);
