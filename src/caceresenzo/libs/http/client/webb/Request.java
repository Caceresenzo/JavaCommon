 ...
 * You can some "real-life" usage examples at <a href="https://github.com/hgoebl/DavidWebb">github.com/hgoebl/DavidWebb</a>. <br>
 */
public class Request {
	 ...
	 * The parameter will be used to create a query string for GET-requests and as the body for POST-requests with MIME-type <code>application/x-www-form-urlencoded</code>. <br>
	 ...
	 * @param value
	 *            the value of the parameter; <code>null</code> will be converted to empty string, Arrays of Objects are expanded to multiple valued parameters, for all other objects to <code>toString()</code> method converts it to String
	 * @return <code>this</code> for method chaining (fluent API)
	 */
	 ...
	 * The parameter will be used to create a query string for GET-requests and as the body for POST-requests with MIME-type <code>application/x-www-form-urlencoded</code>. <br>
	 * If you use this method, you don't have to call {@link #multipleValues()}, but you should not mix using {@link #param(String, Object)} and this method for the same parameter name as this might cause unexpected behaviour or exceptions.
	 *
	 * @param name
	 *            the name of the parameter (it's better to use only contain ASCII characters)
	 * @param values
	 ...
	 *            the value of the parameter; <code>null</code> will be converted to empty string, for all other objects to <code>toString()</code> method converts it to String
	 * @return <code>this</code> for method chaining (fluent API)
	 */
	 ...
	 *
	 * @param name
	 *            name of the header (HTTP-headers are not case-sensitive, but if you want to override your own headers, you have to use identical strings for the name. There are some frequently used header names as constants in {@link Webb}, see HDR_xxx.
	 * @param value
	 *            the value for the header. Following types are supported, all other types use <code>toString</code> of the given object:
	 *            <ul>
	 ...
	 * Using this method together with {@link #param(String, Object)} has the effect of <code>body</code> being ignored without notice. The method can be called more than once: the value will be stored and converted to bytes later. <br>
	 ...
	 * <li>{@link java.lang.String}, HTTP header 'Content-Type' will be set to TEXT, if not set; Text will be converted to UTF-8 bytes.</li>
	 * <li><code>byte[]</code> the easiest way for DavidWebb - it's just passed through. HTTP header 'Content-Type' will be set to BINARY, if not set.</li>
	 * <li>{@link java.io.File}, HTTP header 'Content-Type' will be set to BINARY, if not set; The file gets streamed to the web-server and 'Content-Length' will be set to the number of bytes of the file. There is absolutely no conversion done. So if you want to upload e.g. a text-file and convert it to another encoding than stored on disk, you have to do it by yourself.</li>
	 * <li>{@link java.io.InputStream}, HTTP header 'Content-Type' will be set to BINARY, if not set; Similar to <code>File</code>. Content-Length cannot be set (which has some drawbacks compared to knowing the size of the body in advance).<br>
	 ...
	 * Your web application should inspect the 'Content-Encoding' header and implement the compression token provided by this client. By now only 'gzip' encoding token is used. If you need 'deflate' create an issue.
	 *
	 * @return <code>this</code> for method chaining (fluent API)
	 */
	...
	/**
	 * See <a href="http://docs.oracle.com/javase/7/docs/api/java/net/URLConnection.html#useCaches"> URLConnection.useCaches</a> <br>
	 * If you don't want your requests delivered from a cache, you don't have to call this method, because <code>false</code> is the default.
	 *
	 * @param useCaches
	...
	/**
	 * See <a href="http://docs.oracle.com/javase/7/docs/api/java/net/URLConnection.html#setIfModifiedSince(long)"> URLConnection.setIfModifiedSince()</a>
	 * 
	...
	/**
	 * See <a href="http://docs.oracle.com/javase/7/docs/api/java/net/URLConnection.html#setConnectTimeout(int)"> URLConnection.setConnectTimeout</a>
	 * 
	 ...
	 * Use this method to set the behaviour for this single request when receiving redirect responses. If you want to change the behaviour for all your requests, call {@link Webb#setFollowRedirects(boolean)}.
	 * 
	 * @param auto
	...
	/**
	 * By calling this method, the HTTP status code is checked and a <code>WebbException</code> is thrown if the status code is not something like 2xx.<br>
	 * <br>
	 * Be careful! If you request resources e.g. with {@link #ifModifiedSince(long)}, an exception will also be thrown in the positive case of <code>304 Not Modified</code>.
	 *
	 * @return <code>this</code> for method chaining (fluent API)
	 */
	 ...
	 * When `waitExponential` is set, then there will be {@link Thread#sleep(long)} between the retries. If the thread is interrupted, there will be an `InterruptedException` in the thrown `WebbException`. You can check this with {@link WebbException#getCause()}. The `interrupted` flag will be set to true in this case.
	 *
	 * @param retryCount
	 *            This parameter holds the number of retries that will be made AFTER the initial send in the event of a error. If an error occurs on the last attempt an exception will be raised.<br>
	 *            Values &gt; 10 are ignored (we're not gatling)
	 * @param waitExponential
	 *            sleep during retry attempts (exponential backoff). For retry-counts more than 3, <tt>true</tt> is mandatory.
	 * @return <code>this</code> for method chaining (fluent API)
	 */
